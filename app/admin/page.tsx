...
import { FileText, MessageSquare, Users, Image as ImageIcon, MessagesSquare } from "lucide-react";

export default async function AdminDashboard() {
  const supabase = await createClient();

  const [{ count: blogCount }, { count: msgCount }, { count: volCount }, { count: imgCount }, { count: pendingCommentCount }] = await Promise.all([
  ...
  const stats = [
    ...
    { label: "Comments Awaiting Review", value: pendingCommentCount ?? 0, icon: MessagesSquare },
  ];
